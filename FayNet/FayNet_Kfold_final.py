...
data_dir = '/content/drive/MyDrive/TCC/dataset'#'/content/drive/MyDrive/TCC/dataset'
...
image_size = (229,220) #era 229,220
batch_size = 8
...
##########################################################################
train_images = train_images.reshape((1356, 229, 220, 1)) #222 #1356# , 229, 220, 1((536, 229, 220, 1))
train_images = train_images.astype('float32') / 255
# arr_ = np.squeeze(train_images[1]) # you can give axis attribute if you wanna squeeze in specific dimension
# plt.imshow(arr_)
# plt.show()
test_images = test_images.reshape((580, 229, 220, 1)) # 94#580#201(134, 229, 220, 1)
...
for train, test in kfold.split(inputs, targets):
  ...
  # metrics = [tf.keras.metrics.TruePositives(thresholds=0.5, name='TP'), tf.keras.metrics.TrueNegatives(thresholds=0.5, name='TN'), tf.keras.metrics.FalsePositives(thresholds=0.5, name='FP'), tf.keras.metrics.FalseNegatives(thresholds=0.5, name='FN'), 'accuracy']
    rms = optimizers.RMSprop(lr=0.0001)#tava 2
    sgd = optimizers.SGD(learning_rate=0.014, nesterov=True)
    # |--- Model training ---|----------------------{{{
    metrics = ['accuracy', tf.keras.metrics.TruePositives(thresholds=0.5, name='TP'), tf.keras.metrics.TrueNegatives(thresholds=0.5, name='TN'), tf.keras.metrics.FalsePositives(thresholds=0.5, name='FP'), tf.keras.metrics.FalseNegatives(thresholds=0.5, name='FN')]

    model.compile(loss='BinaryCrossentropy',optimizer=sgd,metrics =metrics)#['accuracy']
    history = model.fit(inputs[train], targets[train], batch_size=90 , epochs=150, verbose=1 , validation_split = 0.3 ) # bat 10 com 200 epocas; verbose=1
                                                                #era20 90
